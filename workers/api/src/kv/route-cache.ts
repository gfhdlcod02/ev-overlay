...
export class RouteCache {
  ...
  async set(
    key: string,
    route: Omit<CachedRoute, 'version' | 'cachedAt' | 'expiresAt'>
  ): Promise<void> {
    try {
      const now = new Date()
  ...
  async getMetadata(key: string): Promise<{ ttl: number; expiration: number } | null> {
    try {
      ...
      return {
        ttl:
          (metadata.metadata as { expiration: number }).expiration - Math.floor(Date.now() / 1000),
 ...
 */
export function createRouteCache(kv: KVNamespace, ttlSeconds?: number): RouteCache {
