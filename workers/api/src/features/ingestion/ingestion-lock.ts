  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (action === 'acquire') {
      const timeoutMs = parseInt(
        url.searchParams.get('timeoutMs') || String(this.defaultTimeoutMs),
        10
      )
...
export async function acquireIngestionLock(
  ...
  const response = await stub.fetch(
    `http://internal/action?action=acquire&lockId=${lockId}&timeoutMs=${timeoutMs}`
  )
 ...
 */
export async function getIngestionLockStatus(env: {
