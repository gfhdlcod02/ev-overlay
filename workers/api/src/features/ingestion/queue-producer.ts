...
export class QueueProducer {
   ...
   */
  async queueBatchProcess(batchId: string, stations: OcmStationRecord[]): Promise<void> {
    const message: QueueMessage = {
      type: 'PROCESS_BATCH',
      jobId: 0,
      batchId,
   ...
   */
  async queueSnapshotWrite(jobId: string, stationIds: number[], r2Key: string): Promise<void> {
    const message: QueueMessage = {
      type: 'WRITE_SNAPSHOT',
      jobId: 0,
      batchId: jobId,
   ...
   */
  async queueCacheInvalidation(stationIds?: number[]): Promise<void> {
    const message: QueueMessage = {
 ...
 */
export function createQueueProducer(queue: Queue<QueueMessage>): QueueProducer {
